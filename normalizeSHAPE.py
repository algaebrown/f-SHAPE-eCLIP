...
#Functions taken from Steve Busan's generateReactivityProfiles.py script in the Shapemapper pipeline
#Not using stderr though, and altered to process multiple replicates, one replicate per column
#profile contains +modRate - -modRate for each nucleotide
#maxBackground for SHAPEMaP is 0.05, but icSHAPE reactivity are much higher; 0.3 seemed equivocal
...
def HMM(bgDepth,rxDepth): #Determine positions of too-low coverage in bg and rx with a HMM
    ...
    rxDepth1d[rxDepth1d<confminDepth] = 0
    rxDepth1d[rxDepth1d>=confminDepth] = 1
    bgDepth1d[bgDepth1d<confminDepth] = 0
    bgDepth1d[bgDepth1d>=confminDepth] = 1
...
def filterProfile(profile,bgSignal,rxDepth,bgDepth,sequence,maxBackground=confmaxBackground,minDepth=confminDepth, trimEnds=True):
...
#There are a lot of outliers with icSHAPE data. Trying filtering out profiles with -0.2>=rates>=0.2
    ...
    #don't need to filter out gaps in MaP data
    profileStart=0
    profileStop=len(profile)-1
    if trimEnds:
        profileStart = 2 #only trimming by 2 bases at each end
        profileStop -= 2
    for i in range(0,len(profile)):
        for j in range(0,len(profile[0])):
            ...
            #    goodNuc = False
            #if sequence[i]=='T' or sequence[i]=='G': #profile[i][j] <= -0.01 or profile[i][j]>=0.01
            #    goodNuc = False #Use this for DMS probing bc it doesnt react with T or G
            if bgDepth[i][j] < minDepth or rxDepth[i][j] < minDepth: #<, or <=??
                goodNuc = False
            ...
            #print(filteredProfile[i][j],sequence[i],goodNuc)
...
def normalizeSHAPEmain(cov,cov5,sequence,trimEnds=True):
    #need sequence of covered area to ignore T/G nucleotides
    #(DMS only probes A and C)
